"use client";

import { useTranslations } from "next-intl";


  
export default function RailingsSection() {
  const t = useTranslations("Offer");
  return (
<section className="max-w-[1280px] mx-auto px-4 py-24 flex flex-col gap-16">
 
  <div className="flex flex-col gap-6">
    <h4 className="text-[#EB4036] text-sm font-semibold uppercase tracking-[4px]">

      {t("descriptionPage.category")}{t("descriptionPage.railings.titleCategory")}
    </h4>
    <h1 className="text-white text-[4rem] leading-[1.1] font-oswald font-bold uppercase max-lg:text-[2.5rem]">
    {t("descriptionPage.railings.header")}
    </h1>
    <div className="w-[12%] h-[5px] bg-gradient-to-r from-[#EB4036] to-transparent opacity-80 mb-2" />
  <p className="text-[#A5A5A5] text-[1.25rem] leading-[2.1rem] tracking-wide max-w-[80ch] font-raleway">
  {t("descriptionPage.railings.description1")}
  </p>
  <p className="text-[#A5A5A5] text-[1.25rem] leading-[2.1rem] tracking-wide max-w-[80ch] font-raleway">
  {t("descriptionPage.railings.description2")}
  </p>
  <p className="text-[#A5A5A5] text-[1.25rem] leading-[2.1rem] tracking-wide max-w-[80ch] font-raleway">
  {t("descriptionPage.railings.description3")}
  </p>
  </div>

  {/* Sekcja korzyści */}
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
    <div className="flex flex-col gap-5">
      <h2 className="text-[2rem] text-white font-semibold uppercase">  {t("descriptionPage.railings.questionTitle")}</h2>
      <ul className="list-disc list-inside text-[#A5A5A5] text-[1.1rem] font-raleway leading-[2rem] tracking-wide">
        <li>{t("descriptionPage.railings.example1")}</li>
        <li>{t("descriptionPage.railings.example2")}</li>
        <li>{t("descriptionPage.railings.example3")}</li>
        <li>{t("descriptionPage.railings.example4")}</li>
        <li>{t("descriptionPage.railings.example5")}</li>
      </ul>
    </div>
    <div className="bg-[#1A1A1A] p-6 rounded-2xl shadow-lg border border-[#2e2e2e]">
      <h3 className="text-[#EB4036] text-xl font-semibold mb-4 uppercase">{t("descriptionPage.railings.materialTitle")}</h3>
      <p className="text-[#A5A5A5] font-raleway text-[1.1rem] leading-[2rem]">
      {t("descriptionPage.railings.mateiralDescription")}
      </p>
    </div>
  </div>

  {/* Sekcja "Before / After" */}
  <div className="flex flex-col gap-10">
    <h2 className="text-[2.5rem] font-semibold uppercase text-white"> {t("descriptionPage.railings.check")} </h2>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="relative overflow-hidden rounded-xl border border-[#2e2e2e] shadow-md">
        <img
          src="/assets/images/before-balustrada.jpg"
          alt="Stara balustrada - Before"
          className="w-full h-auto object-cover"
        />
        <div className="absolute top-4 left-4 bg-[#EB4036] text-white text-sm px-3 py-1 rounded-full uppercase font-semibold tracking-wide">
        {t("descriptionPage.railings.before")}
        </div>
      </div>
      <div className="relative overflow-hidden rounded-xl border border-[#2e2e2e] shadow-md">
        <img
          src="/assets/images/after-balustrada.jpg"
          alt="Nowa balustrada - After"
          className="w-full h-auto object-cover"
        />
        <div className="absolute top-4 left-4 bg-green-600 text-white text-sm px-3 py-1 rounded-full uppercase font-semibold tracking-wide">
        {t("descriptionPage.railings.after")}
        </div>
      </div>
    </div>
    <p className="text-[#A5A5A5] text-[1.1rem] leading-[2rem] tracking-wide max-w-[85ch] font-raleway mt-4">
    {t("descriptionPage.railings.efectDescription")} </p>
  </div>

  {/* Call To Action */}
  <div className="bg-gradient-to-r from-[#EB4036] to-[#9e1c12] rounded-xl p-10 text-white flex flex-col items-start gap-4 shadow-xl mt-20">
    <h2 className="text-[2.5rem] font-semibold uppercase max-lg:text-[1.8rem]">{t("descriptionPage.railings.contactTitle")}</h2>
    <p className="text-lg font-raleway leading-[2rem]">
    {t("descriptionPage.railings.contactDescription")}
    </p>
    <a
      href="/contact-me"
      className="mt-4 inline-block px-6 py-3 bg-white text-[#EB4036] font-semibold rounded-xl uppercase tracking-wider hover:bg-[#f8f8f8] transition-all duration-200"
    >
     {t("descriptionPage.railings.contactButtonLabel")}
    </a>
  </div>
</section>



 


  );
}
